...
export function useSkills(initialFilters = {}) {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([skillsApi.getAll(filters), skillsApi.getCategories()])
      .then(([skillsRes, catRes]) => {
        // axios: data is in res.data or res.data.data
        const sd = skillsRes.data?.data || skillsRes.data;
        const cd = catRes.data?.data    || catRes.data;

        const skillsList = Array.isArray(sd) ? sd : (sd?.skills || []);
        setSkills(filters.page === 1 ? skillsList : (prev) => [...prev, ...skillsList]);
...
export function useUserProfile(userId) {
  ...
  useEffect(() => {
    if (!userId) return;
    setLoading(true);
    Promise.all([usersApi.getById(userId), reviewsApi.getForUser(userId)])
